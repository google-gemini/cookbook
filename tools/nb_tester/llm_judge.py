    ...
    def evaluate_cell_output(
        ...
        if not new_output.strip():
            ...
            has_print_or_call = any(
                "print(" in l or "display(" in l or "interaction" in l or "generate" in l
                for l in code_lines
            )
            ...
            if len(old_output.strip()) > 30:
                return SemanticComparisonResult(
                    verdict="REGRESSION",
                    explanation="New output is empty while reference output had substantial content.",
                    is_regression=True
                )

        model_name = self.config.OUTPUT_JUDGE_MODEL
        system_instruction = (
            "You are an expert AI output regression judge for Google Gemini API Cookbook notebooks.\n"
            "You are given the Python CODE of a notebook cell, the OLD REFERENCE OUTPUT saved in the repo, "
            "and the NEW OUTPUT generated by executing the cell.\n"
            "Evaluate whether the new output is a valid, correct response to the code/prompt or a regression.\n"
            "Classify into:\n"
            "- 'MATCH': The outputs are semantically equivalent or achieve the same goal.\n"
            "- 'SLIGHT_VARIATION': The output has minor phrasing/formatting differences, but is completely valid and correct.\n"
            "- 'REGRESSION': The new output completely failed, hallucinated nonsense, returned an error, ignored prompt instructions, or degraded significantly.\n"

...
* **Prohibited use policy:** If a PR appears to involve a restricted use case under [Google's Generative AI Prohibited Use Policy](https://policies.google.com/terms/generative-ai/use-policy), flag it for maintainer review. Do not approve it until the concern is resolved.
...
* **Vertex AI workflows and administration:** Redirect notebooks specifically focused on enterprise Vertex AI infrastructure, pipelines, or Google Cloud administration to [GoogleCloudPlatform/generative-ai](https://github.com/GoogleCloudPlatform/generative-ai). Direct SDK bugs to [googleapis/python-genai](https://github.com/googleapis/python-genai).
* **Google AI Studio:** Direct questions or PRs about Google AI Studio, including API key generation or project settings, to the official Google AI Studio documentation or support channels.
* **Promotional content:** Flag PRs that primarily promote a paid or proprietary third-party service for maintainer review. Third-party integrations must provide clear technical learning value.

## Dependencies and third-party frameworks

* **Native SDK first:** Use the official `google-genai` SDK and Python standard libraries for core model capabilities, such as function calling, structured outputs, chat history, system instructions, and embeddings.
* **Avoid unnecessary orchestration frameworks:** Do not introduce third-party wrapper or orchestration frameworks, such as `langchain`, `llamaindex`, `crewai`, or `litellm`, when `google-genai` supports the demonstrated capability.
